...
const Requests: React.FC = () => {
  ...
  const filteredRequests = (requests ?? [])
    ...
    .filter(r => {
      ...
      return r.request_title.toLowerCase().includes(term)
        || r.client?.full_name?.toLowerCase().includes(term)
        || r.designer?.full_name?.toLowerCase().includes(term);
    })
    .sort((a, b) => {
      ...
      if (sortConfig.key === "client") {
        ...
        aVal = a.designer?.full_name ?? "";
        bVal = b.designer?.full_name ?? "";
      } else {
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 p-6 overflow-auto">
            ...
            <div className="flex space-x-2 overflow-x-auto pb-2 hide-scrollbar">
              {["all","pending","approved","rejected"].map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab as typeof activeTab)}
                  className={`px-3 py-2 rounded-lg text-xs font-medium ${activeTab===tab?"bg-teal-100 text-teal-800":"bg-white text-gray-600 hover:bg-gray-100"}`}>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            {isLoading ? <div className="p-6 text-center">Loading requests...</div>
                  ...
                  <thead className="bg-gray-50">
                    <tr>
                      {["request_title","client","designer","status","created_at"].map(col => (
                        <th key={col} onClick={() => handleSort(col as keyof RequestType | "client" | "designer")}
                          className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100">
                          <div className="flex items-center">{col.toUpperCase()} {sortConfig.key===col && <ArrowUpDown className="ml-1 h-4 w-4"/>}</div>
                        </th>
                      ))}
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredRequests.map(request => (
                      <tr key={request._id.toString()} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">{request.designer?.full_name || "Unassigned"}</div>
                          <div className="text-sm text-gray-500">{request.designer?.email || ""}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap"><StatusBadge status={request.status} /></td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatTimeAgo(request.created_at)}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <button onClick={()=>openRequestModal(request)} className="text-teal-600 hover:text-teal-900 px-3 py-1 rounded-md hover:bg-teal-50 transition-colors">View</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
